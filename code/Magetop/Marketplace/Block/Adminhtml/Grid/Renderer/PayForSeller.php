 ...
 **/
namespace Magetop\Marketplace\Block\Adminhtml\Grid\Renderer;
...
class PayForSeller extends AbstractRenderer
{
    ...
    public function _getValue(\Magento\Framework\DataObject $row)
    {
        ...
        /*$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $dataSaleOrder = $objectManager->create('Magetop\Marketplace\Model\Saleslist')->load($saleOrderId);
        ...
        $html = '<button type="button" class="button view_transaction" tran_id="'.$row->getId().'" onclick="Magetop.viewTransaction('.$row->getId().')" title="'.__('View Transaction').'"><span><span><span>'.__('View Transaction').'</span></span></span></button>';
        return $html;
    }    
}