...
abstract class Reviewactions extends \Magento\Backend\App\Action
{
    ...
    {
        $_preparedActions = array('index', 'grid', 'new', 'edit', 'save', 'delete', 'config', 'massStatus', 'massDelete');
    ...
    */
    protected function _massDeleteAction()
    {
        ...
        try {
            ...
            $error = true;
            $this->messageManager->addException($e, __('We can\'t delete review of '.strtolower($model->getOwnTitle()).' right now. '.$e->getMessage()));
        }

        if (!$error) {
            $this->messageManager->addSuccess(
                __($model->getOwnTitle(count($ids) > 1).' have been deleted.')
            );
        }
