...
class Save extends \Magetop\Marketplace\Controller\Product\Account {
    ...
    /**
     * @var Magetop\Marketplace\Controller\Product\Initialization\Helper
     */
     ...
     * @param \Magetop\Marketplace\Controller\Product\Initialization\Helper $initializationHelper
     */
    public function __construct(
		...
		\Magetop\Marketplace\Controller\Product\Initialization\Helper $initializationHelper,
	...
	public function execute(){
		...
		if($data){
			try {
				...
				if(\Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\Marketplace\Helper\Data')->getProductApprovalRequired()){
					...
					$this->messageManager->addSuccess(__('Product submitted.'));
				}
				...
				if(!$productIdCheck){
                    ...
                    $this->_objectManager->create('Magetop\Marketplace\Helper\EmailSeller')->sendProductAdminEmail($product->getName(), \Magento\Framework\App\ObjectManager::getInstance()->create('Magento\Customer\Model\Session')->getId(), 'added');
				}else{
				    $this->_eventManager->dispatch(
						'controller_action_catalog_product_savepost_entity_after',
                    ...
                    $this->_objectManager->create('Magetop\Marketplace\Helper\EmailSeller')->sendProductAdminEmail($product->getName(), \Magento\Framework\App\ObjectManager::getInstance()->create('Magento\Customer\Model\Session')->getId(), 'edited');
				}
            ...
            if (@$redirectBack === 'draft') {
                ...
                if($objectManager->create('Magetop\Marketplace\Helper\Data')->getProductApprovalRequired()){
                    $product_status = 0;
                }else{
                    $product_status = 1;
                }					
        ...
        // Sanitize input data
    			...
    			if($newProduct->getTypeId()==ConfigurableProduct::TYPE_CODE){
    				...
    				$_model1=$this->_objectManager->create('Magetop\Marketplace\Model\ResourceModel\Products');
    				$_arrayparam=array();
    				$_arrayparam[]=$_params;
    				foreach($productids as $key=>$val){
    					$_model2=$this->_objectManager->create('Magetop\Marketplace\Model\Products')->getCollection()->addFieldToFilter('product_id',$_params['product_id'])
                ...
                if($moduleManager->isEnabled('Magetop_SellerMembership') && \Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\Marketplace\Helper\Data')->getSellerMembershipIsEnabled()){
                    $membershipData = \Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\SellerMembership\Model\SellerMembership')
                                                                                                 ...
                                                                                                 ->getFirstItem();
                    if($membershipData['id']){
                        $seller_membership_old = \Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\SellerMembership\Model\SellerMembership')->load($membershipData['id']);
