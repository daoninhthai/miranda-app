...
class EmailSeller extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function sendNewOrderEmail(\Magento\Sales\Model\Order $order,$sellerId)
    {
        ...
        try {
            $transport = $this->_transportBuilder->setTemplateIdentifier(
                ...
                [
                    'email' => \Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\Marketplace\Helper\Data')->getAdminEmail(),
                    'name'  => 'Admin Notification',
                ]
            )->addTo(
                $customer->getData('email'), $customer->getData('firstname') . ' ' . $customer->getData('lastname')
            )->getTransport();
    ...
    public function sendRegisterSellerEmail($customer)
    {
        ...
        try {
            $transport = $this->_transportBuilder->setTemplateIdentifier(
                ...
                \Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\Marketplace\Helper\Data')->getAdminEmail(), 'Admin'
            )->getTransport();
            $transport->sendMessage();
        } catch (\Magento\Framework\Exception\MailException $ex) {
    ...
    public function sendRequestWithdrawEmailToSeller($data)
    {
        ...
        //data 
        $paymentDetail = $objectManager->create('Magetop\Marketplace\Model\Payments')->load($data['payment_id']);                
        $this->inlineTranslation->suspend();
        try {
            $transport = $this->_transportBuilder->setTemplateIdentifier(
     ...
     * @param id customer
     */
    public function sendProductSellerEmail($product_name, $customer_id, $status)
    {
        $storeId = $this->_storeManager->getStore()->getId();
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
    	$customer = $objectManager->create('Magento\Customer\Model\Customer')->load($customer_id);
        ...
        try {
            $transport = $this->_transportBuilder->setTemplateIdentifier(
                'marketplace_general_email_product_vendor'
            )->setTemplateOptions(
                ['area' => \Magento\Framework\App\Area::AREA_FRONTEND, 'store' => $storeId]
            )->setTemplateVars(
                [
    ...
    public function sendProductAdminEmail($product_name, $customer_id, $status)
    {
        $storeId = $this->_storeManager->getStore()->getId();
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
    	$customer = $objectManager->create('Magento\Customer\Model\Customer')->load($customer_id);
        ...
        try {
            $transport = $this->_transportBuilder->setTemplateIdentifier(
                'marketplace_general_email_product_admin'
            )->setTemplateOptions(
                ['area' => \Magento\Framework\App\Area::AREA_FRONTEND, 'store' => $storeId]
            )->setTemplateVars(
                [
                    'myvar1' => $customer->getData('firstname') . ' ' . $customer->getData('lastname'),
                    'myvar2' => $this->_storeManager->getStore()->getUrl('rj2kzun1'),
                    'product_name' => $product_name,
                    'status' => $status
                ]
            )->setFrom(
