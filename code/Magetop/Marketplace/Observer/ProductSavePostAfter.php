...
class ProductSavePostAfter implements ObserverInterface
{
	...
	public function execute(\Magento\Framework\Event\Observer $observer){
        if($observer->getActionMk() == 'add'){
            ...
            $customerData=\Magento\Framework\App\ObjectManager::getInstance()->create('\Magento\Customer\Model\Session')->getCustomer()->getData();
            $product_id=$observer->getProductId();
            $_modelmk=\Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\Marketplace\Model\Products')->getCollection()
                                                                                        ->addFieldToFilter('product_id',$product_id)
                                                                                        ->addFieldToFilter('user_id',$customerData['entity_id'])
                                                                                        ->getFirstItem();
			$_modelmk =$_modelmk->getData();
			$_modelmk = (array)$_modelmk;
			$_mkId = $_modelmk['id'];
			// die;
			$_model = \Magento\Framework\App\ObjectManager::getInstance()->create('Magetop\Marketplace\Model\Products')->load($_mkId);
            $_model->setModified(date('Y-m-d H:i:s',$this->_timezone->scopeTimeStamp()));
            if($_model->getStatus() == 1){
                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                if($objectManager->create('Magetop\Marketplace\Helper\Data')->getProductUpdateApprovalRequired()){
                    $product_status = 0;
                    $product = $objectManager->create('Magento\Catalog\Model\Product')->load($product_id);
                    $product->setStatus(2);
                    $product->save();
