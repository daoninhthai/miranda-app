/**
* Magento Support Team- http://magetop.com
* Menu Creator Pro
* Version 2.0
	...
	*/
	...
	MCP = {
		...
		*/
		...
		updateGroupTree : function(id) {
			var url = $("#baseurl").val() + "menupro/index/updatemenu";
			var groupId, parentId, pLiId,
				liId, tempId, depth;
			var data = "";
			$("#sitemap li.group-"+id+", #sitemap li.group-"+id+ " li").each(function() {
				tempId = $(this).attr("id");
				if(tempId != 0){
		...
		addChild : function( id, groupId ) {
			...
			$('#parent_id option').each(function() {
				if( $(this).val() == id ){
					...
					$('#edit_form').attr('action', baseurl + 'admin/menupro/menu/save/' + newAction);
					$('#type').focus();
				}
			});
		...
		*/
			...
			$.ajax({
				...
				dataType: 'json'
					...
					if(data != ""){
						...
						menuId = data.menu_id;
						groupId = data.groupmenu_id;
						title = data.title;
						image = data.image;
						type = data.type;
						parentId = data.parent_id;
						urlValue = data.url_value;
						imageStatus = data.image_status;
						permission = data.permission;
						status = data.is_active;
						target = data.target;
						storeids = data.storeids;
						dropdownColumns = data.dropdown_columns;
						iconClass = data.icon_class;
						hideSubHeader = data.hide_sub_header;
						autosub = data.autosub;
						classSubfix = data.class_subfix;
						useCategoryTitle = data.use_category_title;
						textAlign = data.text_align;
						hidePhone = data.hide_phone;
						hideTablet = data.hide_tablet;
										...
										textAlign, hidePhone, hideTablet, description)
		{
			...
			if(image != ""){
				...
				baseurl + 'pub/media/' + image + '"><img width="22" height="22" class="small-image-preview v-middle" alt="' +
				image + '" title="' + image + '" id="image_image" src="' + baseurl + 'pub/media/' + image + '"></a>';
    ...
    // TODO: add loading state handling
		...
		expandAll : function(id) {
			...
			var liOpen = "sm_liOpen";
			var liClosed = "sm_liClosed";
			$('#group-' + id).toggleClass('Opened');
			if($('#group-' + id).hasClass('Opened')){
				$("#sitemap li.group-"+id+", #sitemap li.group-"+id+ " li").each(function() {
				liClass = $(this).attr('class');
