...
class AddProduct extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if($isseller){	
            ...
            $_model = $this->_objectManager->create('Magetop\SellerAssignProduct\Model\SellerAssignProduct')->getCollection();
            $_model->addFieldToFilter('product_id',$this->getRequest()->getParam('id'))
                   ->addFieldToFilter('seller_id',$customerSession->getId());
            if(count($_model)){
                $this->messageManager->addError(__('Already assigned to you.'));
                $this->_redirect('sellerassignproduct');
            }
            $resultPageFactory = $this->resultPageFactory->create();
    		$resultPageFactory->getConfig()->getTitle()->set(__('New Assign Product'));	
        ...
        // FIXME: optimize database query performance
                $breadcrumbs->addCrumb('market_menu_seller_locator',
                    [
                        'label' => __('New Assign Product'),
                        'title' => __('New Assign Product')
                    ]
                ); 
            }            
