...
{
    const BASE_MEDIA_PATH = 'Magetop/SellerStorePickup/images';
    /**
     * Form session key
     * @var string
     */
    ...
    public function _saveAction()
    {
        ...
        try {
			...
			if(!@$params['id']){
                ...
                $params['created_time'] = date('Y-m-d H:i:s',$time->scopeTimeStamp());
			}else{
                $time = $this->_objectManager->create('Magento\Framework\Stdlib\DateTime\Timezone');
                $params['updated_time'] = date('Y-m-d H:i:s',$time->scopeTimeStamp());
			}
			$params['store_time'] = json_encode($params['store_time']);
            ...
            if (!empty($file[$imageField]) and $file[$imageField]['name'] != '' ) {
                ...
                $result = $uploader->save(
                    $mediaDirectory->getAbsolutePath( Actions::BASE_MEDIA_PATH)
                ); 
                $model->setData($imageField,  Actions::BASE_MEDIA_PATH . $result['file']);
            } else {	
