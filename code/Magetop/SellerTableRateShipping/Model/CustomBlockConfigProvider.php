 ...
 **/
namespace Magetop\SellerTableRateShipping\Model;
...
class CustomBlockConfigProvider implements ConfigProviderInterface
{
    ...
    public function __construct(
        ...
        \Magetop\SellerTableRateShipping\Model\SellerTableRateShippingFactory $mkShipping
    ) {
    ...
    {
        $dataShippingSeller = [];
        $enabled = $this->scopeConfiguration->getValue('carriers/sellertablerate/active', ScopeInterface::SCOPE_STORE);
            ...
            if(!in_array($item->getProduct()->getTypeId(),$productNotWeight)){
                ...
                if($mkProductData && $mkProductData->getId())
        		{
                    ...
                    if(@$infoBuyRequest['assignproduct_id']){
                        ...
                        $weightProductSeller[$mkProductData->getUserId()][] =  $item->getWeight()*$item->getQty();
                    }
                    //end Assign product
                }
            }
